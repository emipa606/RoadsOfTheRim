...
namespace RoadsOfTheRim
{
    ...
    public class WorldObjectComp_ConstructionSite : WorldObjectComp
    {
        ...
        public CompProperties_RoadsOfTheRimConstructionSite Properties => (CompProperties_RoadsOfTheRimConstructionSite)props;
        ...
        {
            if (!costs.TryGetValue(name, out var value))
            {
        ...
        public float GetPercentageDone(string name)
        {
            ...
            return (float)(costTotal - leftTotal) / costTotal;
        }
        ...
        public static string CostModifersDescription(int fromTile_int, int toTile_int, ref float totalCostModifier)
        {
            ...
            GetCostsModifiers(fromTile_int, toTile_int, ref elevationModifier, ref hillinessModifier, ref swampinessModifier, ref bridgeModifier);
        ...
        public override void CompTick()
        {
            try
            {
                ...
                var percentOfWorkLeftToDoAfter = ((float)GetLeft("Work") - amountOfWork) / GetCost("Work");
                foreach (var resourceName in DefModExtension_RotR_RoadDef.allResources)
                {
                    ReduceLeft(resourceName, (int)Math.Round((float)GetLeft(resourceName) - (percentOfWorkLeftToDoAfter * GetCost(resourceName))));
                }
        ...
        public string ResourcesAlreadyConsumed()
        {
            ...
            try
            {
                foreach (var resourceName in DefModExtension_RotR_RoadDef.allResourcesAndWork)
                {
                    ...
                    resourceList.Add(string.Format("{0} {1}", GetCost(resourceName) - GetLeft(resourceName), resourceName));
                }
            }
            catch
            {
        ...
        public void SetCosts()
        {
            try
            {
                ...
                // Total cost modifier
                var totalCostModifier = (1 + elevationModifier + hillinessModifier + swampinessModifier + bridgeModifier) * ((float)settings.BaseEffort / 10);
        ...
        public string ProgressDescription()
        {
            ...
            if (parentSite.helpFromFaction != null)
            {
                ...
                {
                    stringBuilder.Append("RoadsOfTheRim_ConstructionSiteDescription_HelpStartsWhen".Translate(string.Format("{0:0.00}", (parentSite.helpFromTick - Find.TickManager.TicksGame) / (float)GenDate.TicksPerDay)));
                }
            }

            // Show total cost modifiers
            var totalCostModifier = 0f;
            stringBuilder.Append(CostModifersDescription(parentSite.Tile, parentSite.GetNextLeg().Tile, ref totalCostModifier));
        ...
        {
            Scribe_Collections.Look(ref costs, "RotR_site_costs", LookMode.Value, LookMode.Value, ref costs_Keys, ref costs_Values);
            Scribe_Collections.Look(ref left, "RotR_site_left", LookMode.Value, LookMode.Value, ref left_Keys, ref left_Values);
        }
    }
}

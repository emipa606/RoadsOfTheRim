...
namespace RoadsOfTheRim
{
    ...
    public class WorldLayer_RoadsOnWater : WorldLayer_Paths
    {
        ...
        public override IEnumerable Regenerate()
        {
            ...
            var roadLayerDefs = DefDatabase<RoadWorldLayerDef>.AllDefs.OrderBy((RoadWorldLayerDef rwld) => rwld.order).ToList();
            var i = 0;
            while (i < grid.TilesCount)
            {
                ...
                if (tile.WaterCovered)
                {
                    ...
                    if (tile.potentialRoads != null)
                    {
                        ...
                        for (var k = 0; k < roadLayerDefs.Count; k++)
                        {
                            var flag = false;
                            list.Clear();
                            for (var l = 0; l < tile.potentialRoads.Count; l++)
                            {
                                ...
                                var layerWidth = road.GetLayerWidth(roadLayerDefs[k]);
                                if (layerWidth > 0f)
                                {
                                    flag = true;
                                }
        ...
        public override Vector3 FinalizePoint(Vector3 inp, float distortionFrequency, float distortionIntensity)
        {
            ...
            var a = new Vector3(roadDisplacementX.GetValue(coordinate), roadDisplacementY.GetValue(coordinate), roadDisplacementZ.GetValue(coordinate));
